...
// For accountant
app.get('/accounting/amount-earned-calculate', { preHandler: [authZ, req => isAccountant(req.user, false)], }, AccountingController.calculateAmountEarned)
app.get('/accounting/statistics-by-days', { preHandler: [authZ, req => isAccountant(req.user, false)], }, AccountingController.getStatisticsByDays)
app.get('/accounting/amount-day-earned-calculate', { preHandler: [authZ, req => isAccountant(req.user, false)], }, AccountingController.calculateAmountDayEarned)
...
// For Admin
app.get('/analytics/negative-balance-users',
  { preHandler: [authZ, req => isAdmin(req.user, false)], },
  AnalyticsController.getNegativeUserBalance)
app.get('/analytics/amount-day-earned-calculate',
  { preHandler: [authZ, req => isAdmin(req.user, false)], },
